...
/* const CustomOrderIcon = React.forwardRef(function refForwardToIcon(
  { type, ...rest },
  ref
) {
  return type === 'check' ? (
    <BsBagCheckFill ref={ref} {...rest} />
  ) : type === 'dash' ? (
    <BsFillBagDashFill ref={ref} {...rest} />
  ) : (
    <BsFillBagFill ref={ref} {...rest} />
  );
});
 */
/* const CustomTooltip = React.forwardRef(function customTTFowardRef(
  { children, ...rest },
  ref
) {
  return (
    <Tooltip ref={ref} bg="red" label="hello" {...rest} offset={[0, 0]}>
      {children}
    </Tooltip>
  );
}); */
                      ...
                      {order.items.map((item) => {
                        return (
                          <Tr key={item.CODIGO + order.id}>
                            ...
                            <Td>{formatPrices(item.PRECIO * item.quantity)}</Td>
                          </Tr>
                        );
                      })}
                    </Tbody>
